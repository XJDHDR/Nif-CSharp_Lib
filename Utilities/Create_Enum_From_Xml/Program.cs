...
if (File.Exists("input.txt"))
{
	...
	for (ushort _i_ = 0; _i_ < _fileLineParts_.Length; _i_++)
	{
		...
		// Find out which value is the longest and store it's length.
		if (_currentLineParts_[1].Length > _longestValueLength_)
		{
			_longestValueLength_ = _currentLineParts_[1].Length;
		}
	}
	...
	for (ushort _i_ = 0; _i_ < _fileLineParts_.Length; _i_++)
	{
		...
		if (_fileLineParts_[_i_][1].Length < _longestValueLength_)	// If this enum's value is shorter than longest, add spaces to pad it out to align all of them.
		{
			_outputStringBuilder_.Append(' ', _longestValueLength_ - _fileLineParts_[_i_][1].Length);
		}

		_outputStringBuilder_.Append(_fileLineParts_[_i_][1]);	// Add the enum value
